...
class SatSolver {
    ...
    public:
        VI solve(const VVI &_clauses) {
            ...
            int level = 0;
            UI to_be_assigned = var(exv);
            VI assigned;

            while (to_be_assigned < value.size()) {
#ifdef DEBUG
                printstatus();
#endif
                related_clause[to_be_assigned] = -1;
                VI asv(1, to_be_assigned << 1);
                while (asv.size()) {
                    int v = asv.back();
                    int vv = var(v);
                    asv.pop_back();
                ...
                while (to_be_assigned < value.size() && ~value[to_be_assigned]) {
                    to_be_assigned++;
                }
                level++;
            }
